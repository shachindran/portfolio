...
function ThailandMark() {
  return (
    <svg className="experience-thailand-map" viewBox="0 0 260 520" aria-hidden="true">
...
export function ExperienceSection() {
  ...
  const titleY = useTransform(smoothProgress, [0, 1], [14, -28]);
  const outputOpacity = useTransform(smoothProgress, [0.4, 0.66], [0, 1]);
  const outputY = useTransform(smoothProgress, [0.4, 0.66], [16, 0]);
  const mapOpacity = useTransform(smoothProgress, [0.02, 0.42], [0.07, 0.14]);
  const mapY = useTransform(smoothProgress, [0, 1], [10, -6]);
    ...
    <section className="experience-section" id="experience">
      <div className="experience-scroll-stage" ref={stageRef}>
        <div className="experience-sticky">
          ...
          <motion.h2 className="experience-title" style={{ y: reduced ? 0 : titleY }} aria-hidden="true">
            BANGKOK
          </motion.h2>
          ...
          <div className="experience-layout">
            ...
            <div className="experience-visual" aria-label="Document formats moving through an extraction workflow">
              <motion.div
                ...
                style={{ opacity: reduced ? 0.11 : mapOpacity, y: reduced ? 0 : mapY }}
              >
                <ThailandMark />
              </motion.div>
              ...
              <motion.div
                ...
                style={{ opacity: reduced ? 1 : outputOpacity, y: reduced ? 0 : outputY }}
              >
      ...
      <div className="experience-mobile">
        ...
        <div className="experience-mobile-docs">
          {docs.map((doc) => (
            <span key={doc.type}><b>{doc.type}</b>{doc.label}</span>
          ))}
        </div>
